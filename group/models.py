...
from utils.choices import ProviderType, groupType, protocolBaseMapChoice
...
class Base_map(models.Model):
    ...
    protocol_carto = models.CharField(
        max_length=5, choices=protocolBaseMapChoice.choices
    )
...
class Layer_provider_style(models.Model):
    ...
    def save(self, *args, **kwargs):
        numberOfLPSOfLayer = (
            Layer_provider_style.objects.filter(layer_id=self.layer_id).count() + 1
        )
